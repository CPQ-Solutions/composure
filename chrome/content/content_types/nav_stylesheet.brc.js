(function() { br_utils.create_plugin ({	
	...
	*/
	regex_urls: ["/admin/navigation/edit_stylesheet.jsp"],
	/**
	...
	function content(params) {
		...
		*/
		...
		this.initiate_display = function() {
			var m = this.get_menu(),
				download_links = jQuery("a.general-text",doc).filter(":contains('Download')"),
		...
		function ajax_submit() {
			...
			br_load.ajax({
				type: "POST",
				multiform: true,
				url: doc.location.protocol+"//"+doc.location.hostname+"/admin/navigation/edit_stylesheet.jsp",
				data: {
					formaction: "generateNavMenuCSS",
					_additionalCSS: file
				},
		...
		*/
		...
		this.get_local_directory = function() { return "Nav_Stylesheets"; };
	}
